...
// Production-ready, mobile-optimized QuestionForm component
// Improved spacing, readable typography, larger tap targets, clean mobile-first layout
    ...
    <div className="mt-4 bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden w-full max-w-full">
      ...
      <div className="bg-gradient-to-r from-brand-primary to-brand-secondary p-4 sm:p-5 text-white">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            onClick={onExit}
            className="px-3 py-1.5 bg-white/25 hover:bg-white/35 transition-all text-xs font-medium rounded-lg self-start sm:self-auto"
          >
            {exit_option?.title || "Exit"}
          </button>
        </div>
        ...
        {progress && (
          <div className="mt-3">
            <div className="flex justify-between text-[11px] sm:text-xs text-white/95 mb-1">
              <span>Progress</span>
              <span>{progress.percentage}%</span>
            </div>
      ...
      <div className="p-4 sm:p-6">
        <p className="text-base sm:text-lg font-medium text-gray-900 mb-4 leading-relaxed">
          {message}
          {required && <span className="text-red-500 ml-1">*</span>}
        </p>
        ...
        {/* TEXT INPUT */}
        {(input_type === "text" || input_type === "number" || input_type === "date") && (
          <form onSubmit={handleSubmit} className="space-y-4 mt-2">
            <input
              type={input_type}
              value={textAnswer}
              onChange={(e) => setTextAnswer(e.target.value)}
              placeholder={input_hint || "Type your answer..."}
              required={required}
              disabled={disabled}
